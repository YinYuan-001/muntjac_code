...
while (my $now_line = <I>){
    chomp;
    next if(/^#/);
    my @a=split(/\s+/,$now_line);
    if($a[2] eq "mRNA"){
    print O $now_line;
    }
    elsif($a[2] eq "CDS"){
    $no++;
    my ($chr,$info,$start,$end,$strand,$phase,$name)=($a[0],$a[1],$a[3],$a[4],$a[6],$a[7],$a[8]);
    # $chr=~s/chr//g;
    $name=~/Parent=([^;]+)/;
    $name=$1;
...
while(my $seq=$fa->next_seq){
    ...
    print O "$chr\t$info\tCDS\t$real_pos\t$real_pos\t.\t$strand\t0\tID=CDS:yy_made_it;Parent=$name\n";
            }
