...
##Using the latest techniq in Collect.pm

die "Usage: $0 <seq.fa> <TE.out|gff|annot> <TE.out|gff|annot> ...\n" if(@ARGV < 2);
...
for(my $i=1;$i<@ARGV;$i++){
	if ($ARGV[$i]=~/\.gff$/){
		...
		Read_RepeatMasker($ARGV[$i],\%rep);
	}elsif ($ARGV[$i] =~ /\.annot$/){
		Read_RepeatProteinMasker($ARGV[$i],\%rep);
	}else{
...
foreach my $seq_name (sort keys %seq) {
	...
	foreach my $p (@{$rep{$seq_name}}) {
		next  if $p->[0]=~/MITE/i;
		next  if $p->[4]=~/(centromere|unknown|MITE)/i;
		push @pos,[$p->[2],$p->[3]];
	}
...
sub Read_RepeatProteinMasker{
	my ($file,$p)=@_;
	open IN,$file or die "$!";
	while(<IN>){
		chomp;
		my @c=split(/\s+/);
		next if ($c[6] ne '+' && $c[6] ne '-');
		my $tname = $c[3];
		my $strand = $c[6];
		my ($start,$end) = ($c[4]<$c[5])?@c[4,5]:@c[5,4];
		my $TE_name = $c[7];
		my $TE_class = $c[8];
		
		push @{$p->{$tname}}, [$TE_name,$strand,$start,$end,$TE_class];
	}
	close IN;
}
