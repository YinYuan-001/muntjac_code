...
#my $maf="Gemsbok.all.maf.swap.maf";#"DF_contig10k2ARS1.maf.swap.maf";
my $maf=$ARGV[0];
...
open S,"> $ARGV[1].breakpoint_info";
print S "chr\tstart\tend\ttype\n";
open L,"> $ARGV[1].record";
open O,"> $ARGV[1].sta";
...
foreach my $wisent_chr(sort keys %link){
	...
	print L "$wisent_chr\t$wisent_start\t$wisent_end\t$wisent_strand\t<->\t$cattle_chr\t$cattle_start\t$cattle_end\t$cattle_strand\n";
    }
    print L "\n";
	...
	if(!$cattle_chr_pre){
        ...
        my $real_start = $cattle_start;if ($cattle_strand eq "-"){$real_start = $cattle_end};
        my $real_end_pre = $cattle_end_pre;if ($cattle_strand_pre eq "-"){$real_end_pre = $cattle_start_pre};
        
        #print S "$wisent_chr\t$wisent_end_pre\t$wisent_start\t$sv_type\n";
        #print S "$wisent_chr\t$wisent_end_pre\t$wisent_start\t$sv_type\t$cattle_chr_pre\t$cattle_start_pre\t$cattle_end_pre\t$cattle_start\t$cattle_end\t($cattle_strand)\t($cattle_strand_pre)\n";
	    print S "$wisent_chr\t$wisent_end_pre\t$wisent_start\t$sv_type\t$cattle_chr_pre\t$real_end_pre\t$real_start\n";
        $cattle_chr_pre=$cattle_chr;
	    $cattle_start_pre=$cattle_start;
	    $cattle_end_pre=$cattle_end;
	    $cattle_strand_pre=$cattle_strand;
